...
public class OrdemServicoRepository implements OrdemServicoDB {
	...
	public OrdemServicoCorretiva buscarPorId(long id) {
		...
		ordem = (OrdemServicoCorretiva) session.get(
				OrdemServicoCorretiva.class, id);
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<OrdemServicoCorretiva> listarPorStatus(Departamento setor,
			Status status) {
		Session session = (Session) HibernateUtil
				.getAttributeRequest("session");
		Criteria crit = session.createCriteria(OrdemServicoCorretiva.class);
	...
	public boolean jaExiste(OrdemServicoCorretiva ordem) {
		...
		ord = (OrdemServicoCorretiva) crit
				.add(Restrictions.ilike("equipamento", ordem.getEquipamento()))
				.add(Restrictions.ilike("componente", ordem.getComponente()))
				.add(Restrictions.ilike("descricaoAcao",
						ordem.getDescricaoAcao()))
	...
	public List<OrdemServicoCorretiva> pesquisarPorFiltros(FiltrosOrdens filtro) {
		...
		if (filtro != null) {
			...
			if (!filtro.getEquipamento().isEmpty()
					&& filtro.getEquipamento() != null) {
				crit.add(Restrictions.ilike("equipamento",
						filtro.getEquipamento(), MatchMode.ANYWHERE));
			}
			if (!filtro.getComponente().isEmpty()) {
				crit.add(Restrictions.ilike("componente",
						filtro.getComponente(), MatchMode.ANYWHERE));
			}
			...
			if (filtro.getTipoTrabalho() != null) {
				crit.add(Restrictions.eq("tipoTrabalho",
						filtro.getTipoTrabalho()));
			}
			...
			if (filtro.getDataInicio() != null && filtro.getDataFinal() != null) {
				crit.add(Restrictions.between("dataCriacao",
						filtro.getDataInicio(), filtro.getDataFinal()));
			}
